...
const res2=await employeeApi.GetFindAndInsertTrack(trackID,request.EMP_NO,selectedDateFrom,to,storedCounter,cc,storedNotes,0)
  ...
  if (!result.ok) {
    ...
    setInfo({ Loading: false, allowed:false,message:"لم يتم إضافة المسار بنجاح!",done:true });
    setLoading(false);
    return;
  }
  else {       
    setLoading(false);
    setUploadVisible(false);
    setInfo({ Loading: false, allowed:false,message:" تمت إضافة المسار بنجاح!!"+
    "\n"+
  ...
  return (
    ...
    {!info.Loading&&info.done && (
         ...
         <Field
              ...
              {startEnabled&&<Field
               ...
               // value={notes}
                ...
                showPlaceholder={true}
                numberOfLines={1}
                placeholder={"سبب استخدام المركبة"}
                style={[styles.name]}             
      ...
      {!hasStarted&&<Button
        ...
        title={startEnabled?"بدء المسار  ":"تم البدء من ساعة "+"selectedDateFrom.toLocaleTimeString()"}
