...
import Card from "../../components/CardElectRow";
import auth from "../../api/auth";
import authStorage from "../../auth/storage";

function ElectricityServicesScreen({ navigation,route }) {
  ...
  const getElectServices = async () => {
    setUnits({ loading: true, data: null });
     const token = await authStorage.getToken();

    const result = await tabletApi.GetElectServicesByBID(B_ID,token); 
    ...
    let data = result.data.Services.sort(function (a, b) { return b.SERVICE_NO - a.SERVICE_NO; }) ;;//.sort(function (a, b) { return b.hitCount - a.hitCount; }) .slice(0, 5);
...
export default ElectricityServicesScreen;
