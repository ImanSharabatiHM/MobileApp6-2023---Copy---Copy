...
function LayersPermissions({ navigation, route }) {
  const [employeeLayersPermissions, setEmployeeLayersPermissions] = useState({ loading: false, data: [] });
  ...
  const GetAllLayersPermissionsForEmp = async () => {
    setEmployeeLayersPermissions({ loading: true, data: [] });
    ...
    const result= await tablet.GetAllLayersForEmp(custId,token);
    //console.log(result);
    if (!result.ok) {
    //  / setError(true);
      setEmployeeLayersPermissions({ loading: false, data:[] });
      return;
    }
  

    setEmployeeLayersPermissions({ loading: false, data:result.data });
    //console.log(data);
  };
  ...
  const handleChange = async (ID,CHECKED ) => {
    ...
    var len=employeeLayersPermissions.data.length;
    for(var i=0;i<len;i++)
    {
      var obj=employeeLayersPermissions.data[i];
      if(ID==obj.NAME){
...
const UpdateEmployeeLayersPermissions = async () => {
  
    data=employeeLayersPermissions.data;
    setLoading(true);
    var token = await authStorage.getToken();
    const result = await tablet.PostUpdateLayersPermissions(token,custId,{"permissions":employeeLayersPermissions.data});
   console.log(result)
     if (!result.ok) {
      console.log(result);
      return;
  ...
  return (
   ...
   />} 
      ...
      <Form
        ...
        onSubmit={()=>GetAllLayersPermissionsForEmp()}
      ...
      {!employeeLayersPermissions.loading && employeeLayersPermissions.data.length==0 && (
          <Info
        ...
        {((showEPaymentFeature||1)&&!employeeLayersPermissions.loading&&employeeLayersPermissions.data.length>0&& 
         ...
         onPress={() => {
            UpdateEmployeeLayersPermissions()
          //GetenrateLink("2");
...
export default LayersPermissions;
