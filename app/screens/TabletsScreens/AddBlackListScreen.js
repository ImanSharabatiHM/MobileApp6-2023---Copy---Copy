...
function AddBlackListScreen({ navigation, route })
{
  const { user } = useAuth();
  ...
  const handleChange1 = async (ForBuilding) => {
    setIsForBuild(ForBuilding == 1 || ForBuilding==3);
        ...
        >
          <ScrollView>
            ...
            <Field
              ...
              />
            <FormRadioButtonGroup title={"إضافة:"} handleChange1={handleChange1} name="ForBuilding"
                items={[{ key: 1, label: "بناء", value: 1 },
                 { key: 2, label: " وحدة داخل البناء", value: 2 },
                 { key: 3, label: "قطعة", value:3 },
                 { key: 4, label: "مالك الوحدة", value:4 }
            ...
            {selectedSignBoard.SerialNo==0&&<SubmitButton  title={"إضافة للقائمة السوداء" }/>}
          </ScrollView>
...
export default AddBlackListScreen;
